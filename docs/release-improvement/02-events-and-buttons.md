...
최신 기준은 [PLAN.md](../../PLAN.md) §3입니다. 아래 ID·속성은 구현 참고이며 새 정책이나 구현 완료 기록이 아닙니다.
...
| `ai_chat_turn1` | 기존→수정 | 현재 대화 에피소드에서 실제 AI 완료 응답 첫 쌍 | episode당 1회; mode/is_resume | **대표 주요 이벤트** |
| `ai_chat_turn3` | 기존→수정 | 실제 AI 완료 응답 3쌍 도달 | episode당 1회; mode/is_resume | 추가 이용 지표 |
| `ai_chat_finish` | 추가 | 사용자가 명시적으로 마무리함 | 마무리 행동 1회; completed_turns | 아니오 |
| `ai_feedback_view` | 추가·도입 확정 | 선택적 도움 평가 1문항을 실제 노출 | 정상 답변을 받은 이용 구간의 명시적 마무리; 5점 평가·건너뛰기 제공 | 아니오 |
...
AI 에피소드는 **이번에 대화방을 열어 시작한 이용 구간**이다. 이전 저장 대화의 턴을 초기값으로 더하지 않는다. 새 구간의 첫 성공 응답과 세 번째 성공 응답을 각각 기록하며, 이어하기는 `is_resume=true`로 나눠 본다. 다시 열기가 많아 퍼널이 부풀지 않도록 사용자 도달률은 사용자/세션 단위로 중복을 제거한다.
...
| StoryCard/StoryDetailModal | `story_edit_open`, `story_hide`, `story_delete_open`, `story_report_open` | 기존 hide는 확정된 개인 숨김/작성자 비공개 ID로 분리, 나머지는 모달 표시 |
...
| CreateStoryModal | `story_adult_check`, `adult_warning_dismiss` | 기존 소스 대조용. 첫 공개 성인 기능은 보류하며 등록·노출·관련 AI 차단과 일치하도록 정리 |
...
| AdultVerificationModal | `adult_self_confirm`, `adult_modal_close` | 첫 공개 기능 보류: 성인 사연 등록·노출·관련 AI를 차단하고 실제 인증 완료 이벤트를 만들지 않음 |
...
| SessionSummaryCard | `ai_mood_before_select`, `ai_mood_after_select` | 제거 대상: 기존 기분·한 줄 소감 입력은 도움 평가 1문항으로 대체 |
| 확정된 선택 평가 | `ai_feedback_submit`, `ai_feedback_skip` | 정상 답변 후 명시적 마무리 시 5점 도움 평가. 저장 성공만 submit; 평가값 GA4 제외; 최대 30일 보관 |
| AIErrorReportModal | `ai_error_report_submit`, `ai_error_report_cancel`, `ai_error_report_close` | 실제 접수 op, 콘솔 출력만 성공 금지 |
| PremiumModal | `premium_info`, `premium_demo`, `premium_later`, `premium_close` | 준비 중 안내. 구매 완료·데모를 통한 실제 AI 이용처럼 표시하지 않음 |
...
| MyPageView 알림 | `notify_balance_toggle`, `notify_vote_toggle`, `notify_comment_toggle` | 준비 중 안내. 알림 전달·구매 성공으로 기록하지 않음 |
...
| 확정된 신규 구분 | `story_hide_for_me`, `story_unhide_for_me`, `story_make_private`, `story_make_public` | 개인 숨김/작성자 비공개 각각 op. 재공개 시 댓글·투표 복원 확정; 즉시 취소와 마이페이지 숨긴 목록 복원 제공 확정 |
...
- 동의한 뒤에만 GA4 태그를 로드한다. 동의 전·거부 상태는 Google 전송 0, 과거 행동 소급 전송 0이다. 설정에서 변경·철회할 수 있어야 한다. 선택적 내부 행동 로그에도 같은 동의 범위를 적용한다.
- 문항: **“이 AI 대화가 도움이 되었나요?”**. 1~5점은 전혀 도움 안 됨 / 별로 도움 안 됨 / 보통 / 조금 도움 됨 / 매우 도움 됨. 건너뛰기를 제공한다.
- 이용 구간별 1건, 버전·모드·저장/만료 시각을 저장한다. 본인·운영자만 접근하며 최대 30일 보관, 대화 삭제·탈퇴 시 함께 삭제한다. 이후 개인 식별 불가 집계만 남긴다.
- 점수는 GA4 제외. 4~5점 수 / 유효 응답 수와 전체 분포, 대상·노출·제출·미응답을 함께 본다. 미응답을 불만족으로 간주하지 않는다.
